...
// Real daily quotes fetched from ./api/market.json (generated by the backend
// pipeline from Stooq — see src/backend/collectors/market_data.py).
export function tickerHTML(quotes = []) {
    if (!quotes.length) return '';
    const strip = [...quotes, ...quotes]; // duplicate for seamless loop
...
// Real feed list fetched from ./api/sources.json (generated from
// src/shared/config/sources/*.yaml — see sources_loader.py).
...
export function sourcesOverlayHTML(sourcesByCategory = {}) {
    const categories = Object.keys(sourcesByCategory).sort();
    const body = categories.length ? categories.map(category => `
        <li class="sources-category">${escapeHTML(category)}</li>
        ${sourcesByCategory[category].map(s => `<li>${escapeHTML(s)}</li>`).join('')}
    `).join('') : '<li>Sources are unavailable right now.</li>';
    return `
