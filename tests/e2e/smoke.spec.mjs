...
test.describe('newspaper frontend smoke', () => {
    ...
    test('loads with no console errors and renders page chrome', async ({ page }) => {
        ...
        // The ticker renders real quotes from ./api/market.json (generated by
        // the backend pipeline); it's legitimately empty until the first
        // pipeline run populates that file. When quotes are present, the
        // ticker duplicates them for the seamless loop, so expect an even
        // count of at least 2.
        const tickerItemCount = await page.locator('.ticker-item').count();
        expect(tickerItemCount % 2).toBe(0);
        if (tickerItemCount > 0) expect(tickerItemCount).toBeGreaterThanOrEqual(2);
